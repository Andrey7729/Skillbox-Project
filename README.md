...
- [Краткая инструкция пользователю](#краткая-инструкция-пользователю)
  * [Проверка](#проверка)
    + [Файл конфигурации config.json](#файл-конфигурации-config.json)
    + [Файл с запросами requests.json](#файл-с-запросами-requests.json)
  * [Запуск](#запуск)
...
Поисковый движок разработан в 7 этапов:
...
**Этап 4. Создание инвертированного индекса**
...
|:-:|:-|:------------------|
|1|Целевые ОС| Windows 10        |
|2|Компилятор| Встроенный MinGW,11,0w64 |
|3|Средство сборки| cmake версии 3.30.5 |
|4|IDE|Clion 2024.3.4|
|5|GIT|Git version 2.45.2|
|6|Библиотека nlohmann_json|[Библиотека](https://github.com/nlohmann/json/releases/download/v3.11.3/json.tar.xz "Nlohmann-JSON - GitHub") версии 3.11.3|)
...
- в корне проекта созданы и заполнены файлы config.json и requests.json.
...
#### Запуск
Для запуска проекта необходимо выполнить конфигурацию CMake файла CMakeLists.txt, находящегося в директории ```source/```, далее выполнить сборку основного (```search_engine```) или тестового (```search_engine_test```) приложения и после этого запустить исполняемый файл.
